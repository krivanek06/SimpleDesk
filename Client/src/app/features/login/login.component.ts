    ...
    if (val.username && val.password) {
      ...
      this.authService.login(val.username, val.password).subscribe((result) => {
          this.userService.loadLoggedInUser().subscribe(subs => {
            ...
            if (result && subs) {
              this.router.navigateByUrl('/request-open');
              this.requestService.activateConnection();
              // this.location.replaceState('/'); // clears browser history so they can't navigate with back button
              // this.router.navigate(['/request-open']); // tells them they've been logged out (somehow)
              // window.location.href = `${environment.request-open}request-open`;
              // this.router.navigate(['request-open'], {relativeTo: this.route});
            }
          });
        }
      );
